...
void RobustDubins::Solver::solve() {
    bool wind = m_problemStatementInput.wind_y || m_problemStatementInput.wind_x;
//    std::cout << "wind y = " << m_problemStatementInput.wind_y << std::endl;
//    std::cout << "wind x = " << m_problemStatementInput.wind_x << std::endl;
//    std::cout << "No wind status = " << wind << std::endl;
    ...
    if (compareEndpoints(m_problemStatementNorm.get_stateInitial(),
        ...
        if (!wind) {
//            std::cout << "Solve no wind conditions" << std::endl;
            solveBBB_noWind();
            solveBSB_noWind();
        }
        else {
...
void RobustDubins::Solver::solveBBB_noWind() {
    // terminal conditions
...
void RobustDubins::Solver::solveBSB_noWind() {
    // terminal conditions
    double x = m_problemStatementNorm.get_xFinal(); // final planar positions
    double y = m_problemStatementNorm.get_yFinal();
    double h = MathTools::mod(m_problemStatementNorm.get_hFinal(),2.0*M_PI);
