...
void build_ocps_table_no_wind(double v_min, double v_max, double u_max) {
    ...
    for (int curr_speed = 0; curr_speed < SPEEDS; ++curr_speed) {
        ...
        for (int orientation = 0; orientation < 2; ++orientation) {
            curr_state.orientation = orientation;
            ...
            for (std::vector<State>::iterator target_it = symmetric_targets->begin(); target_it != symmetric_targets->end(); ++target_it) {
                Coordinate target_coordinate = id_to_coordinate((*target_it).neighbour_id, orientation);
                ...
                double hFinal;
                if (orientation == 0) {
                    hFinal = (*target_it).orientation * 2 * M_PI / ORIENTATIONS;
                }
                else if (orientation == 1) {
                    if ((*target_it).orientation == 0) {
                        hFinal = 7 * 2 * M_PI / ORIENTATIONS;
                    }
                    else {
                        hFinal = ((*target_it).orientation - 1) * 2 * M_PI / ORIENTATIONS;
                    }
                }
                ...
                for (std::vector<int>::iterator s_it = subset->begin(); s_it != subset->end(); ++s_it) {
                    ...
                    candidate.target.row = target_coordinate_rot.row;
                    candidate.target.col = target_coordinate_rot.col;
                    candidate.h_final = (*target_it).orientation;
                    ...
                    if (candidateID < 10) {
                        ...
                        double cost = vsdsolver.solveRobust(candidate.path_type, candidate.path_orientation, candidateID, robustX, robustY);
                        if (cost > -1) { // has solution
                            candidate.valid = 1;
                            candidate.cost = cost;
                            get_path_collision(candidate.reduced_path, *robustX, *robustY, candidate.path_type, orientation);
                            solveStatus = 1;
                        }
                        ...
                        if (solveStatus) {
                            ...
                            get_path_collision(candidate.reduced_path, pathHistory, switchingPts, pathType, orientation);
                        }
                    }
...
extern "C" {
    ...
    VAR_SPEED_API void build_ocps_table(double v_min, double v_max, double u_max, double wind_x, double wind_y) {
        ...
        Wind windIn(wind_x, wind_y);
        int count = 0;

        std::cout << "v_min = " << v_min << std::endl;
        std::cout << "wind_x = " << wind_x << std::endl;
        std::cout << "wind_y = " << wind_y << std::endl;
        std::cout << "Wind = (" << windIn.x << ", " << windIn.y << ")\n";

        /// No wind. Calculate all transitions with symmetry and rotation
        bool wind = windIn.x || windIn.y;
        if (! wind) {
            build_ocps_table_no_wind(v_min, v_max, u_max);
            return;
        }

        /// Wind conditions. Calculate all transitions without symmetry and rotation
        ...
        for (auto & neighbor : neighbors) {
            double xFinalIn = neighbor[0];
            double yFinalIn = neighbor[1];
            for (int startOrientation = 0; startOrientation < ORIENTATIONS; startOrientation++) {
                for (int orientFinalIn = 0; orientFinalIn < ORIENTATIONS; ++orientFinalIn) {
                    //define the possible paths not including dubins for optimization
                    const int CandidateIDs[CANDIDATES] = {4, 5, 6, 7, // (B)S(B) Robust dubins paths with LSL, LSR, RSL, RSR
                                                          40, 41, 42, 43, // (BCB)(B) with LL,LR,RL,RR
                                                          48, 49, 50, 51, // (B)(BCB) with LL,LR,RL,RR
                                ...
                                if (candidateID < 10) {
                                    ...
                                    if (cost > -1) { // has solution
                                        ...
                                        if (startOrientation > 1) {
                                            for (std::vector<Coordinate>::iterator it = candidate.reduced_path->begin();
                                                 it != candidate.reduced_path->end(); ++it) {
...
//                                            for (std::vector<Coordinate>::iterator pait = candidate.path_cells->begin();
//                                                 pait != candidate.path_cells->end(); ++pait) {
...
//                                    }
                    ...
                    count++;
                    std::cout << count << "/512 Done" << std::endl;
                }
            }
//            count++;
//            std::cout << count << "/512 Done" << std::endl;
