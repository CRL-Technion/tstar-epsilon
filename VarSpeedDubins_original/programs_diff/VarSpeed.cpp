...
typedef struct Candidate {
	...
	std::vector<Coordinate2>* reduced_path = new std::vector<Coordinate2>;
} Candidate;
...
void save_to_file(std::map <long, Candidate>& ocps_table, double v_min, double v_max, double u_max) {
	...
	if (myfile.is_open()) {
		...
		for (std::map <long, Candidate>::iterator mit = ocps_table.begin(); mit != ocps_table.end(); ++mit) {
			...
			myfile << "speeds=[";
			for (std::vector<Coordinate2>::iterator vit = mit->second.reduced_path->begin(); vit != mit->second.reduced_path->end(); ++vit) {
...
void copy_candidate(Candidate& source, Candidate& target) {
	...
	for (std::vector<Coordinate2>::iterator it = source.reduced_path->begin(); it != source.reduced_path->end(); ++it) {
		target.reduced_path->push_back(*it);
	}
}

void build_ocps_table(double v_min, double v_max, double u_max, double xFinalIn, double yFinalIn, int orientFinalIn, int startOrientation) {
    ...
    std::vector<State2> *symmetric_targets = new std::vector<State2>;
    ...
    Coordinate2 target_coordinate = id_to_coordinate(target_it.neighbour_id, startOrientation);
...
//            target_it.orientation = orientFinal;
            ...
            //for (std::vector<State2>::iterator target_it = symmetric_targets->begin(); target_it != symmetric_targets->end(); ++target_it) {
...
//					State2 curr_state_rot = curr_state;
//					State2 target_rot = target_it;
...
//					}
//					State2 target_mirror = target_it;
...
//						}
//						curr_state_rot.orientation += 2;
//						Coordinate2 target_coordinate_rot = id_to_coordinate(target_rot.neighbour_id, 0);
...
/*int main() {
	...
	int orientFinal = 2;
	int startOrient = 1;
	build_ocps_table(v_min, v_max, u_max, xFinal, yFinal, orientFinal, startOrient);
	return 0;
}*/