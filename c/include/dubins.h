...
typedef struct dubinsCoordinate {
	int row;
	int col;
} dubinsCoordinate;
...
typedef struct dubinsCandidate {
	bool valid = false;
	long key = 0;
	double cost = -1;
	transition trans;
//	relativeTrajectory transition;
	double path[4] = {};
	std::vector<dubinsCoordinate> reduced_path = {};
}dubinsCandidate;
...
class Dubins {
    public:
        ...
        void vanilaDubinsPath(Robot *robot, State s, State g, Wind mapWind, double *out);

        void dubinsSegment(double param, double segInit[3], int seg_type, double r, double *out);

        void dubinsPathSample(Robot *robot, State s, double *path, double t, double *out);

        bool pathValidityCheck(Robot *robot, Map *map, State s, double *path, double cost);

        int getDubinsSegmentNPoints(Robot *robot, double *path);

        void getDubinsSegmentPoints(Robot *robot, State start, double *path, int n_points, double *points_out);

        void obstacleDubinsPath(Robot *robot, Map *map, State s, State g, double *out);

        long dubinsKey(transition trans, int pathId);

        void findCellsOfPath(Robot *robot, Map *map, State s, double *path, double cost,
                             std::vector<dubinsCoordinate> &cellsVec);

        bool pathValidityByCells(Map *map, State s, dubinsCandidate cand);

        void copyDubinsCandidate(dubinsCandidate &source, dubinsCandidate &target);
...
//void vanilaDubinsPath(Robot* robot, State s, State g, double* out)
//{
...
//void dubinsTest(Robot* robot, Map* map){
...
//    absThGoal = (double)g.theta * (2 * M_PI) / robot->getNumOfOrientation();
//
//    cout << "atan " << atan2(dy, dx) << endl;
//    cout << "atan2(dy, dx) + (2 * M_PI) " << atan2(dy, dx) + (2 * M_PI) << endl;
//    cout << "fmod(atan2(dy, dx) + (2 * M_PI), 2 * M_PI) " << fmod(atan2(dy, dx) + (2 * M_PI), 2 * M_PI) << endl;
//
//    theta = fmod(atan2(dy, dx) + (2 * M_PI), 2 * M_PI); // difference between stat and goal orientation
//    alpha = fmod(absThStart - theta + (2 * M_PI), 2 * M_PI); // fix start orientation to the difference
//    beta = fmod(absThGoal - theta + (2 * M_PI), 2 * M_PI); //fix goal orientation to the difference
//    cout << "theta,alpha,beta: " << theta << "," << alpha << "," << beta << endl;
//
//    dubinsLSL(alpha, beta, d, r, array);
//    cout << "LSL: " << array[0] << "," << array[1] << "," << array[2] << endl;
//    for (i = 0; i < 3; i++) {
//        wayLength[LSL][i] = array[i];
//    }
//
//    dubinsLSR(alpha, beta, d, r, array);
//    cout << "LSR: " << array[0] << "," << array[1] << "," << array[2] << endl;
//    for (i = 0; i < 3; i++) {
//        wayLength[LSR][i] = array[i];
//    }
//
//    dubinsRSL(alpha, beta, d, r, array);
//    cout << "RSL: " << array[0] << "," << array[1] << "," << array[2] << endl;
//    for (i = 0; i < 3; i++) {
//        wayLength[RSL][i] = array[i];
//    }
//
//    dubinsRSR(alpha, beta, d, r, array);
//    cout << "RSR: " << array[0] << "," << array[1] << "," << array[2] << endl;
//    for (i = 0; i < 3; i++) {
//        wayLength[RSR][i] = array[i];
//    }
//
//    dubinsRLR(alpha, beta, d, r, array);
//    cout << "RLR: " << array[0] << "," << array[1] << "," << array[2] << endl;
//    for (i = 0; i < 3; i++) {
//        wayLength[RLR][i] = array[i];
//    }
//
//    dubinsLRL(alpha, beta, d, r, array);
//    cout << "LRL: " << array[0] << "," << array[1] << "," << array[2] << endl;
//    for (i = 0; i < 3; i++) {
//        wayLength[LRL][i] = array[i];
//    }
//
...
//                    bestWay = i;
//                    bestCost = cost;
//                    cout << "path " << i << " is valid" << endl;
//                }
//            }
//        }
//    }
//    cout << "best way is: " << bestWay << endl;
...
//void findCellsOfPath(Robot* robot, Map* map, State s, double* path, double cost, std::vector<dubinsCoordinate> &cellsVec) {
