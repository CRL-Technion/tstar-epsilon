...
void VarSpeedDubins::Solver::set_problemStatement(VarSpeedDubins::ProblemStatement vsdprob){
  ...
  m_rdprob_bang.set_wind(m_vsdprob.get_wind_x(), m_vsdprob.get_wind_y());
  ...
  m_rdprob_corn.set_wind(m_vsdprob.get_wind_x(), m_vsdprob.get_wind_y());
...
double VarSpeedDubins::Solver::solveRobust(std::string& pathType, std::string& pathOrientation, int robustID, std::vector<double>*& x, std::vector<double>*& y) {
    m_rdsolver_bang.solve();
    m_rdsolver_corn.solve();
    RobustDubins::Path sol;

    if (robustID == 4) { //(B)S(B) LSL
        pathType = "B-S-B";
        pathOrientation = "LSL";
        sol = m_rdsolver_bang.get_LSL();
    }
    else if (robustID == 5) { //(B)S(B) LSR
        pathType = "B-S-B";
        pathOrientation = "LSR";
        sol = m_rdsolver_bang.get_LSR();
    }
    else if (robustID == 6) { //(B)S(B) RSL
        pathType = "B-S-B";
        pathOrientation = "RSL";
        sol = m_rdsolver_bang.get_RSL();
    }
    else if (robustID == 7) { //(B)S(B) RSR
        pathType = "B-S-B";
        pathOrientation = "RSR";
        sol = m_rdsolver_bang.get_RSR();
    }
    else if (robustID == 8) { //(C)(C)(C) LRL
        pathType = "C-C-C";
        pathOrientation = "LRL";
        sol = m_rdsolver_corn.get_LRL();
    }
    else if (robustID == 9) { //(C)(C)(C) RLR
        pathType = "C-C-C";
        pathOrientation = "RLR";
        sol = m_rdsolver_corn.get_RLR();
    }

    std::string statusString = sol.get_solutionStatus();

    sol.set_spacing(0.05);
    if ((statusString.compare("feasible") == 0) ||
        (statusString.compare("optimal") == 0)) {
        std::vector<double> xHistory = sol.get_xHistory();
        for (std::vector<double>::iterator it = xHistory.begin(); it != xHistory.end(); ++it) {
            x->push_back(*it);
        }


        double xFinal = sol.get_xFinal();
        x->push_back(xFinal);

        std::vector<double> yHistory = sol.get_yHistory();
        for (std::vector<double>::iterator it = yHistory.begin(); it != yHistory.end(); ++it) {
            y->push_back(*it);
        }
        double yFinal = sol.get_yFinal();
        y->push_back(yFinal);
...
void VarSpeedDubins::Solver::solveAll(){
  ...
  for (int i = 0; i < m_numPaths; i++){
    ...
    vsdcand.set_wind(m_vsdprob.get_wind_x(), m_vsdprob.get_wind_y());
  ...
  std::cout << "wind " << m_rdsolver_bang.get_LSL().wind_x << std::endl;
